...
<body>
    ...
    <jsp:include page="/includes/adminLeft.jsp" />
		<c:set var="s1" value="${dateCreateString}"/>
		...
		<header>
				...
				<span class="head">公告管理區</span>
				<span class="sub1">目前總資料筆數: ${datas}</span>
				<span class="sub2">上次更動日期 : ${s2}</span>
				<span class="sub2">
				<a class="button" href="<c:url value='/admin/createNoticeData.jsp'/>">
...
<%-- 		<td class="getNum">${element.number}</td> --%>
		<td>${element.title}</td>
		<td>${element.date}</td>
		<c:set var="string1" value="${element.updateDate}"/>
		<c:set var="string2" value="${fn:substring(string1,0,16)}"/>
		<td>${string2}</td>
		<td>
		<a href="${pageContext.request.contextPath}/admin/modifyNoticeData.jsp?num=${element.number}&page=${currentPage}">
    			<button class="edit">修改</button>
    		</a>
     	<a href="${pageContext.request.contextPath}/news.controller/ModNotice?action=delete&number=${element.number}&page=${currentPage}">
...
<%-- 				</c:if> --%>
				...
				<c:forEach var="j" begin="1" end="${pages}">
					<li class="pg_num">
						<a href="../news.controller/NewsQueryList?action=notice&page=${j}" >
							<c:out value="${j}"/>
						</a>
					</li>	
					<script>
